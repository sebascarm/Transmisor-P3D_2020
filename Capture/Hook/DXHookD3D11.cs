...
namespace Capture.Hook
{
    ...
    internal class DXHookD3D11: BaseDXHook
    {
        ...
        {
            this.DebugMessage("Hook: Begin (seba)");
        ...
        //Variables
        //bool Swap_Principal_Cargado = false;
        ...
        int PresentHook(IntPtr swapChainPtr, int syncInterval, SharpDX.DXGI.PresentFlags flags)
        {
            ...
            try
            {
                ...
                if (this.Request != null)
                {
                    if (swapChainPtr == Swap_Array[ID_usar])
                        ...
                        //this.DebugMessage(swapChainPtr.ToString());       
                        ...
                        using (Texture2D currentRT = Texture2D.FromSwapChain<Texture2D>(swapChain, 0))
                        {
                            ...
                            if (_resolvedRTKeyedMutex != null) _resolvedRTKeyedMutex.Acquire(0, int.MaxValue); // USADO
                            currentRT.Device.ImmediateContext.CopySubresourceRegion(currentRT, 0, null, _resolvedRT, 0);
                            if (_resolvedRTKeyedMutex != null) _resolvedRTKeyedMutex.Release(1);    // USADO
                            sourceTexture = _resolvedRTShared;  // SE USA ESTE
                            ...
                            ThreadPool.QueueUserWorkItem(new WaitCallback((o) =>
                            {
                                ...
                                if (acquireLock && _resolvedRTKeyedMutex_Dev2 != null)
                                    _resolvedRTKeyedMutex_Dev2.Acquire(1, int.MaxValue); // USADO
                                lock (_lock)  {
                                    ...
                                    if (acquireLock && _resolvedRTKeyedMutex_Dev2 != null)  {
                                        _resolvedRTKeyedMutex_Dev2.Release(0);          // USADO
                                    }
                                    ...
                                    try
                                    {
                                        ...
                                        try
                                        {
                                            using (MemoryStream ms = new MemoryStream())
                                            {
                                                ...
                                                ms.Position = 0;
                                                //  Envio de proceso (modificado)
                                                //  ProcessCapture(ms, _requestCopy, swapChainPtr);
                                                //  Envio de Proceso en nuevo hilo
                                                new Thread(() => { ProcessCapture(ms, _requestCopy, swapChainPtr); }).Start();
                                            }
                                        }
                                        finally
                                        {
            ...
            // probablemente aca es el envio // creo q es el retorno que tiene q existir
