    ...
    internal abstract class BaseDXHook: SharpDX.Component, IDXHook
    {
        ...
        protected void SendResponse(Screenshot response)
        {
            System.Threading.Tasks.Task.Factory.StartNew(() =>
            {
                ...
                {
                    // Modificado               
                    Interface.SendScreenshotResponse(response, (IntPtr)0);
        ...
        protected void ProcessCapture(Stream stream, ScreenshotRequest request, IntPtr swapChainPtr)
        {
            ProcessCapture(ReadFullStream(stream), request, swapChainPtr);
        }

        protected void ProcessCapture(byte[] bitmapData, ScreenshotRequest request, IntPtr swapChainPtr)
        {
            try
            {
                if (request != null)
                {
                    Interface.SendScreenshotResponse(
                        new Screenshot(request.RequestId, bitmapData){Format = request.Format,},
                        swapChainPtr);
                }
                LastCaptureTime = Timer.Elapsed;
            }
